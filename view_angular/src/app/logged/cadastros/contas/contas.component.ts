...
export class ContasComponent implements OnInit {
  ...
  preencherFormGroup(conta: ICadastro): FormGroup {
    return new FormGroup({
      ...
      senha: new FormControl(conta.senha, [Validators.required]),
      senhaConfirmar: new FormControl(conta.senhaConfirmar, [Validators.required]),
    });
  }
  ...
  enviar() {
    ...
    console.log(atualizacaoCliente)
    console.log(atualizacaoConta)

    if (!this.estaEditando()) {
      this.contasService.cadastrar(conta).subscribe((result) => {
